classdef PushArmImplicitTrajectoryOptimization < DirectTrajectoryOptimization
  % phi, lambda
  properties
    nC
  ...
  methods
    function obj = PushArmImplicitTrajectoryOptimization(plant,N,duration,options)
    ...
    function obj = addDynamicConstraints(obj)
      ...
      % [dzy] mu is coeff of friction, ignored for right now
...
%           % [dzy] LCP constraints for friction cone, ignored for now
...
%           obj = obj.addConstraint(lincompl_constraints{i},[lambda_inds;gamma_inds]);
        ...
        if obj.nJL > 0 % [dzy] shouldn't reach here, we don't really have joint limits
...
%       end
      ...
      function [f,df] = nonlincompl_fun(y)
        ...
        q = x(1:nq);
        v = x(nq+1:nq+nv);
        % [dzy] add in contact constraints here, fill out rest of
        % nonlincompl_fun to solve for f, df
     ...
     function [f,df] = dynamics_constraint_fun(obj,h,x0,x1,u,lambda,lambda_jl)
        ...
        % [dzy] Need to write Jacobian for contact dynamics
...
%       obj.nC = obj.plant.getNumContactPairs;
      % [dzy] manually spec that there is one constraint
      obj.nC = 1;
      % [dzy] Took this out, as plant.contactConstraints don't exist
